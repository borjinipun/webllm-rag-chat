...
- You'll help users understand complex topics through clear explanations`
  ...
  /**
   * @param {string} prompt
   * @param {string|null} context - Optional RAG context to inject
   */
  async generateResponse(prompt, context = null) {
    ...
    try {
      ...
      if (context) {
        systemContent +=
          `\n\nUse the following excerpts from the user's uploaded document to answer their question. ` +
          `Be specific and cite page numbers where relevant. ` +
          `If the answer cannot be found in the excerpts, say so clearly.\n\n` +
          `<document_context>\n${context}\n</document_context>`
      }

      // Create messages array with (optionally augmented) system prompt
      const messages = [
      ...
      // Append collapsible sources block if RAG context was used
      if (context) {
        this._appendRagSources(
          this.elements.output.querySelector('.assistant-message:last-child'),
          context,
        )
      }
  ...
  /**
   * Append a collapsible source-excerpts block to an assistant message element.
   * @param {HTMLElement|null} msgEl
   * @param {string} context - Raw context string (newline-separated excerpts)
   * @private
   */
  _appendRagSources(msgEl, context) {
    if (!msgEl) return
    ...
    summary.textContent = 'View source excerpts'
    details.appendChild(summary)

    // Parse context into individual excerpts
    const excerpts = context.split('\n\n---\n\n')
    excerpts.forEach((excerpt) => {
      const lines = excerpt.trim().split('\n')
      const headerLine = lines[0] // e.g. "[Excerpt 1 — Page 3]"
      const bodyText = lines.slice(1).join('\n').trim()

      const chunkEl = document.createElement('div')
      chunkEl.className = 'rag-source-chunk'

      const pageEl = document.createElement('div')
      pageEl.className = 'rag-source-page'
      pageEl.textContent = headerLine

      const bodyEl = document.createElement('div')
      bodyEl.textContent = bodyText

      chunkEl.appendChild(pageEl)
      chunkEl.appendChild(bodyEl)
